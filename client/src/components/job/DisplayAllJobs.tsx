...
import { fetchAllJobs } from "../../features/redux/slice/user/getAllJobsSlice";
import {
  distinctTitleLocationSalary,
  filterJobs,
} from "../../features/axios/api/user/jobDetails";
import { JobsInterface } from "../../types/JobsInterface";
import { isApplied } from "../../features/axios/api/user/applyJobs";
import UserSideJobListingShimmer from "../shimmer/UserSideJobListingShimmer";
import Joblist from "./Joblist";
import JobDetails from "./JobDetails";

function DisplayAllJobs(this: any) {
      ...
      if (isFiltered) {
        filteredJobs = await Promise.all(
          (filtered ?? [])?.map(async (job: JobsInterface) => {
            const jobStatus = await isApplied(job?._id, user?.id);
            console.log("jobstatussssssssssss", jobStatus);
        ...
        console.log(jobsList, "jobliosttttttttttttt");
        filteredJobs = await Promise.all(
          (jobsList ?? [])?.map(async (job: JobsInterface) => {
            const jobStatus = await isApplied(job?._id, user?.id);
            console.log(jobStatus.status, "dataaaaaaaaaaaaa");
        ...
        <Navbar className="mx-auto max-w-screen-2xl px-4 py-3">
          <div className="flex flex-wrap items-center justify-between gap-y-4 text-blue-gray-900">
            ...
            <div className="sm:col-span-1">
              <select
                className="focus:ring-2 focus:ring-blue-gray-600 border-2 rounded-lg py-2 px-4 text-gray-500"
